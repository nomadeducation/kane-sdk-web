...
exports.metadata = async function () {
    const res = await this.api.head("/posts");
...
exports.exists = async function (id) {
    const res = await this.api.head(`/posts/${id}`);
 ...
 * @returns {Promise<*>}
 */
exports.create = async function (post = {}) {
    const res = await this.api.post("/posts", post);
    return res.data;
};
...
exports.update = async function (id, infos = {}) {
    const res = await this.api.patch(`/posts/${id}`, infos);
    return res.status === 200;
};
...
exports.import = async function (values) {
    const res = await this.api.post("/posts/import", values);
    return res.status === 200;
};
