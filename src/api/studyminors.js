...
exports.metadata = async function () {
    const res = await this.api.head("/studyminors");
...
exports.exists = async function (id) {
    const res = await this.api.head(`/studyminors/${id}`);
    return res.status === 200;
};

/**
 * @param {Object} studyMinor
 * @returns {Promise<*>}
 */
exports.create = async function (studyMinor = {}) {
    const res = await this.api.post("/studyminors", studyMinor);
    return res.data;
};
...
exports.get = async function (id) {
    const res = await this.api.get(`/studyminors/${id}`);
    return res.data;
};
...
exports.list = async function (offset = 0, limit = 100) {
    ...
    const res = await this.api.get("/studyminors", {
        headers: {
...
exports.update = async function (id, infos = {}) {
    const res = await this.api.patch(`/studyminors/${id}`, infos);
    return res.status === 200;
};
...
exports.remove = async function (id) {
    const res = await this.api.delete(`/studyminors/${id}`);
    return res.status === 200;
};
...
exports.import = async function (values) {
    const res = await this.api.post("/studyminors/import", values);
    return res.status === 200;
};
