...
exports.metadata = async function () {
    const res = await this.api.head("/degrees");
...
exports.exists = async function (id) {
    const res = await this.api.head(`/degrees/${id}`);
...
exports.get = async function (id) {
    const res = await this.api.get(`/degrees/${id}`);
    return res.data;
};
...
exports.list = async function (offset = 0, limit = 100) {
    ...
    const res = await this.api.get("/degrees", {
        headers: {
...
exports.update = async function (id, infos = {}) {
    const res = await this.api.patch(`/degrees/${id}`, infos);
    return res.status === 200;
};
...
exports.remove = async function (id) {
    const res = await this.api.delete(`/degrees/${id}`);
    return res.status === 200;
};
...
exports.import = async function (values) {
    const res = await this.api.post("/degrees/import", values);
    return res.status === 200;
};
