...
exports.metadata = async function () {
    const res = await this.api.head("/specialties");
...
exports.exists = async function (id) {
    const res = await this.api.head(`/specialties/${id}`);
 ...
 * @returns {Promise<*>}
 */
exports.create = async function (specialty = {}) {
    const res = await this.api.post("/specialties", specialty);
    return res.data;
};
...
exports.update = async function (id, infos = {}) {
    const res = await this.api.patch(`/specialties/${id}`, infos);
    return res.status === 200;
};
...
exports.remove = async function (id) {
    const res = await this.api.delete(`/specialties/${id}`);
    return res.status === 200;
};
...
exports.import = async function (values) {
    const res = await this.api.post("/specialties/import", values);
    return res.status === 200;
};
