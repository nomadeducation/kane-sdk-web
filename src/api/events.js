...
exports.exists = async function (id) {
    const res = await this.api.head(`/events/${id}`);
...
exports.update = async function (id, infos = {}) {
    const res = await this.api.patch(`/events/${id}`, infos);
    return res.status === 200;
};
...
exports.remove = async function (id) {
    const res = await this.api.delete(`/events/${id}`);
    return res.status === 200;
};
...
exports.import = async function (values) {
    const res = await this.api.post("/events/import", values);
    return res.status === 200;
};
